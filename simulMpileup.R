...
for (i in 1:length(tmp)) {
	...
	if (length(tmp2)>1) {
		ncopy <- c(ncopy, rep(tmp2[1], times=tmp2[2])) #2x80 => 2 2 2 2 2....
	} else {
...
for(samIdx in 1:nsams){
    ...
    sampledLams <- runif(nsites, min=rangeLams[1], max=rangeLams[2]) #uniform distribution
...
qqVector <- sample(x=seq(0,Ne,1),size=opt$sites,prob=pder,replace=TRUE)/Ne #seq(0,Ne,1) including 0 and 1
